...
public class AutofillSelectBox extends SelectBox implements PluginWebSupport, AceFormElement, AdminLteFormElement {
    ...
    public String renderTemplate(FormData formData, Map dataModel, String template) {
        ...
        @Nonnull final List<FormRow> optionsMap = getOptionsMap(formData)
                .stream()
                .peek(r -> {
        ...
        Map<String, String> fieldsMapping = generateFieldsMapping(rootForm, isLazyMapping(), (Object[]) getProperty("autofillFields"));
    ...
    protected Map<String, String> generateFieldsMapping(Form rootForm, boolean lazyMapping, Object[] autofillFields) {
        if (lazyMapping) {
            ...
            final FormData formData = new FormData();
            return FormDataUtil.elementStream(rootForm, formData)
                    .filter(element -> {
                        String id = element.getPropertyString(FormUtil.PROPERTY_ID);
                        return !(element instanceof SubForm || element instanceof Column || element instanceof Section || element instanceof FormButton) && !id.isEmpty() && !id.equals(selectBoxId);
                    })
                    .map(e -> e.getPropertyString(FormUtil.PROPERTY_ID))
...
//				LogUtil.info(getClassName(), "form ["+FormUtil.findRootForm(element).getPropertyString("id")+"] element ["+id+"] type ["+types.get(id)+"]");
    ...
    protected boolean isLazyMapping() {
        return "true".equalsIgnoreCase(getPropertyString("lazyMapping"));
    }
