<div class="form-cell form-group" ${elementMetaData!}>
    ...
    <#else>
        <select id="${elementId}" <#if element.properties.multiple! == 'true'>multiple</#if> name="${elementParamName!}" class="js-select2 <#if error??>form-error-cell</#if>" <#if element.properties.readonly! == 'true'> disabled </#if>>
